...
export const thirdpartyscholars = async (SchoolID,selectedClass,selectedSection) => {
  ...
  try {
    ...
    // const data = await makeApiRequest(`${apiUrls?.thirdpartyscholars}?schoolId=${SchoolID}&{`{selectedClass?"class=${selectedClass}":""}`}&section=${selectedSection}`, option);
    return data;
  } catch (error) {
    console.error("Error fetching registrations:", error);
    throw error; // Re-throw the error to allow the component to handle it.
...
export const thirdpartyphotorecords = async (SchoolID,selectedClass, selectedSection) => {
  try {
    const option = {
      method: "GET", // Ensure the method is GET
    };

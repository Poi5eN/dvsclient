 ...
 * @param {number|string} totalPaidAmountFromParent - The pre-calculated total paid amount.
 * @param {number|string} totalDuesAmountFromParent - The pre-calculated total dues amount.
 * @param {string} [filename='fee-data.pdf'] - The desired filename for the downloaded PDF.
 */
const generatePdf = (data, columns, totalPaidAmountFromParent = 0, totalDuesAmountFromParent = 0, filename = 'fee-data.pdf') => {
    ...
    const parsedPaidAmount = parseFloat(totalPaidAmountFromParent);
    console.log("[generatePdf] Received totalPaidAmountFromParent:", totalPaidAmountFromParent, "Type:", typeof totalPaidAmountFromParent);
    ...
    if (!isNaN(parsedPaidAmount)) {
        ...
        console.warn(`[generatePdf] Could not parse paid amount '${totalPaidAmountFromParent}'. Using 0.`);
    }
    ...
    const parsedDuesAmount = parseFloat(totalDuesAmountFromParent);
    console.log("[generatePdf] Received totalDuesAmountFromParent:", totalDuesAmountFromParent, "Type:", typeof totalDuesAmountFromParent);
    ...
    if (!isNaN(parsedDuesAmount)) {
        ...
        console.warn(`[generatePdf] Could not parse dues amount '${totalDuesAmountFromParent}'. Using 0.`);
    }
...
//  * @param {Array<Object>} data - The array of data objects for the table body.
//  * @param {Array<Object>} columns - The column definitions for the table (e.g., [{ header: 'Name', dataKey: 'name' }]).
//  * @param {number|string} totalPaidAmountFromParent - The pre-calculated total amount to display on the summary page. Should be a number or a string representing a number.
//  * @param {string} [filename='table-data.pdf'] - The desired filename for the downloaded PDF.
//  */
// const generatePdf = (data, columns, totalPaidAmountFromParent = 0,dues=0, filename = 'fee-data.pdf') => {
//     // Ensure data is an array, even if a single object is passed
//     const dataArray = Array.isArray(data) ? data : [data];
...
//     const paidAmount = parseFloat(totalPaidAmountFromParent);

...
function AllStudent() {
  ...
  const getStudent = async () => {
    ...
    setIsLoader(true);
    try {
      let classes = selectedClass === "all" ? "" : selectedClass;

      // if(selectedClass && selectedSection){
      const response = await thirdpartyscholars(
        SchoolID,
        classes,
        selectedSection
      );
      // const response = await thirdpartyadmissions(SchoolID,selectedClass,selectedSection);
      if (response.success) {
  ...
  }, [selectedClass, selectedSection, reRender]);
  if (!SchoolID) {
    return (
      <div className="text-center mt-10 text-red-500 font-semibold">
        Kindly Select School
      </div>
    );
  }
  return (
              ...
              >
                <div className="text-gray-700 font-semibold text-sm absolute top-2 right-2 gap-2">
                  <div className="flex justify-center items-center gap-3">
                    <p>
                      {val?.assignedThirdParty == null ? (
                        <span className="bg-[#f0592e] px-1 text-white shadow-md rounded-md text-[8px]">
                          Admin
                        </span>
                      ) : (
                        <span className="bg-[#2fa7db] text-[8px] px-1 rounded-md shadow-md text-white">
                          T-Party
                        </span>
                      )}
                    </p>

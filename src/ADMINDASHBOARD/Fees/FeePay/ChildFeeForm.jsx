...
const ChildFeeForm = ({ child, index, isSelected, showForm, handleChildSelection }) => {
  const {
    formData,
    setFormData,
    setResponseData,
  ...
  const handleMonthMultiSelectChange = (name, selectedOptions) => {
    ...
    structuredMonthlyAddFees.forEach((fee) => {
      const availableFeeOption = currentChildData.availableAdditionalFees.find((opt) => opt.name === fee.name && opt.type === "Monthly");
      if (availableFeeOption) {
        const feeDetail = currentChildData.additionalFeeDetails.find((fd) => fd.name === fee.name && fd.type === "Monthly");
        if (!feeDetail) return;
    ...
    const existingNonMonthlyFees = currentChildData.selectedAdditionalFees.filter((fee) => fee.type !== "Monthly");
    updatedFormData[index].selectedAdditionalFees = [...newSelectedAdditionalFees, ...existingNonMonthlyFees];
    setFormData(updatedFormData);
  };
  ...
  const handleDynamicMultiSelectChange = (field, selectedOptions) => {
    ...
    if (field === "selectedAdditionalFees") {
        ...
        .map((opt) => {
          ...
          if (originalFee) {
            return {
              ...
              dueMonths: originalFee.type === "Monthly" ? currentChildData.selectedMonths.map((m) => m.value) : [],
            };
          }
          return null;
        })
      ...
      const autoSelectedMonthly = currentChildData.selectedAdditionalFees.filter(
        (fee) => fee.type === "Monthly" && !newSelectedAdditionalFees.some((nf) => nf.id === fee.id)
      );
  ...
  const validateFormData = (childFormData, child) => {
    ...
    const payableExcludingDuesFines =
      calculateNetPayableAmount() - (parseFloat(childFormData.pastDues) || 0) - (parseFloat(childFormData.lateFine) || 0);
    ...
    if (
      ...
      toast.warn(
        `Amount paid for ${child.studentName} (₹${totalAmount.toFixed(2)}) exceeds past dues and late fines (Total ₹${onlyPayingDuesAndFines.toFixed(2)}), but no specific month or other fee is selected. Please select the items being paid for or adjust the amount. If this is an advance payment, please add a remark.`
      );
    }
    return true;
  };

  const calculateNetPayableAmount = () => {
    ...
    total += data.selectedAdditionalFees.reduce((sum, fee) => {
      if (fee.type === "Monthly" && fee.dueMonths?.length > 0) {
        return sum + fee.dueMonths.reduce((monthSum, month) => {
          const feeDetail = data.additionalFeeDetails.find((fd) => fd.name === fee.name && fd.type === "Monthly");
          if (feeDetail) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (!validateFormData(currentFormData, child)) return;

    setIsLoader(true);
    const additionalFeesPayload = [];
    const selectedMonthNames = currentFormData.selectedMonths.map((m) => m.value);

    currentFormData.selectedAdditionalFees.forEach((fee) => {
      if (fee.type === "Monthly" && fee.dueMonths?.length > 0) {
        fee.dueMonths.forEach((monthName) => {
          const monthStatus = currentFormData.feeInfo?.monthlyStatus?.find((m) => m.month === monthName);
    ...
    currentFormData.selectedOneTimeFees.forEach((fee) => {
      additionalFeesPayload.push({ name: fee.name });
    });

    const payload = {
      studentId: child.studentId,
      session: useFeePayContext().session,
      paymentDetails: {
        regularFees: currentFormData.selectedMonths.map((monthState) => ({ month: monthState.value })),
        additionalFees: additionalFeesPayload,
        pastDuesPaid: 0,
        lateFinesPaid: 0,
        concession: parseFloat(currentFormData.concession) || 0,
        totalAmount: parseFloat(currentFormData.totalAmount) || 0,
        date: moment(currentFormData.date, "YYYY-MM-DD").format("DD-MM-YYYY"),
        paymentMode: currentFormData.paymentMode,
        transactionId: currentFormData.transactionId || undefined,
        chequeNumber: currentFormData.chequeBookNo || undefined,
        remark: currentFormData.remarks || "",
      },
    };
  ...
  const selectedMonthValues = currentFormData.selectedMonths.map((monthState) => ({
  ...
  const selectedAdditionalFeeValues = currentFormData.selectedAdditionalFees
    ...
    .map((selectedFee) => {
      ...
      return {
        name: availableOption ? availableOption.name : `${selectedFee.name} (${selectedFee.type}) - ₹${selectedFee.amount}`,
        code: selectedFee.id,
      };
    });
  ...
  const selectedOneTimeFeeValues = currentFormData.selectedOneTimeFees.map((fee) => {
    ...
    return {
      name: availableOption ? availableOption.name : `${fee.name} (Due: ₹${fee.dueAmount.toFixed(2)})`,
      code: fee.name,
    };
  });
        ...
        <label id={`child-label-${index}`} className="flex-grow cursor-pointer" htmlFor={`child-checkbox-${index}`}>
          ...
          <div className="flex flex-wrap justify-start items-center gap-x-4 text-xs mt-1">
            ...
            {currentFormData?.pastDues > 0 && (
              <span className="text-purple-600 font-medium">Past Dues: ₹{currentFormData?.pastDues?.toFixed(2)}</span>
            )}
            {currentFormData?.lateFine > 0 && (
              <span className="text-orange-600 font-medium">Late Fine: ₹{currentFormData?.lateFine?.toFixed(2)}</span>
            )}
      ...
      <div className={`transition-all duration-500 ease-in-out overflow-hidden ${showForm ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0"}`}>
        {showForm && (
          <div className="px-4 py-4 border-t flex flex-col lg:flex-row gap-6 bg-white">
            <form onSubmit={handleSubmit} className="flex-grow lg:w-2/3 space-y-5 mb-6 lg:mb-0" noValidate>
              <div className="border rounded-md p-3 bg-gray-50 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-1">
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Select consecutive months with dues.</p>
                </div>
                <div className="md:col-span-1">
                  ...
                  <DynamicMultiSelect
                    ...
                    handleChange={(name, opts) => handleDynamicMultiSelectChange("selectedAdditionalFees", opts)}
                    value={selectedAdditionalFeeValues}
                    requiredClassName="required-fields"
                    containerClassName="w-full"
                    menuClassName="w-full min-w-[200px] whitespace-normal"
                  />
                  <p className="text-xs text-gray-500 mt-1">Monthly fees auto-selected with months.</p>
                </div>
                <div className="md:col-span-1">
                  ...
                  <DynamicMultiSelect
                    ...
                    handleChange={(name, opts) => handleDynamicMultiSelectChange("selectedOneTimeFees", opts)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <ReactInput
                  type="number"
                  label={`Total Amount to Pay (*) ${selectedChildrenIndices.length > 1 ? `(for ${child.studentName})` : ""}`}
            ...
            <PaymentSummary index={index} calculateNetPayableAmount={calculateNetPayableAmount} />
          </div>
        )}
      </div>
    </div>

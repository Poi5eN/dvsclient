...
import { editStaff, getstaffBYID } from "../../../Network/AdminApi";
...
const EditStaff = () => {
  ...
  const fetchEmployees = async () => {
    try {
      const response = await getstaffBYID(email)
     console.log("response",response)
      if (response) {
       
        // setSubmittedData(response?.employees[0]);
        setStaffData(response?.employees[0]);
  ...
  // useEffect(() => {
  //   axios.get(
  //     `https://api.digitalvidyasaarthi.in/api/v1/adminRoute/getAllEmployees?email=${email}`,
  //     {
...
const response=await editStaff(data,email)
if(response?.success){
...
navigate("/admin/staff");
  ...
  toast.error(response?.message)
    ...
    // axios.put(`https://api.digitalvidyasaarthi.in/api/v1/adminRoute/updateEmployee`, data, {
    //   withCredentials: true,
    //   headers: {
    //     Authorization: `Bearer ${authToken}`,

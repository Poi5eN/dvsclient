...
import { BreadcrumbList } from "@chakra-ui/react";
        ...
        {examData?.map((exam, index) => (
          <div key={index} className="bg-white rounded-lg shadow-md p-4 transition-transform hover:scale-105">
            ...
            <h3 className="text-lg font-semibold mb-2">{exam.name}</h3>
             <td className="p-2"> Date :  {exam?.startDate || "N/A"}<span className="text-blue-500 font-extrabold"> TO </span>{exam?.endDate|| "N/A"}</td>
                    ...
                    <div className="flex space-x-1 items-center">
                      ...
                      <span>
                        {/* {formatDate(subject?.examDate)} */}
                         {moment(subject?.examDate).format("DD-MM-YYYY")}
                      </span>
                      {/* <FaClock className="text-gray-500 text-xs" /> */}
                      <span>
                        {/* {subject?.startTime} to {subject?.endTime} */}
                         {moment(subject?.startTime).format("hh-mm A")}  <span className="text-blue-700 font-bold">TO</span>  {moment(subject?.endTime).format("hh-mm A")}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <tbody>
              {examData?.map((val, ind) => (
                <tr key={ind} className="border">
                  ...
                  <td className="p-2">{val?.startDate || "N/A"}<span className="text-blue-500 font-extrabold"> TO </span>{val?.endDate|| "N/A"}</td>

...
const Lanscape = () => {
    ...
    // --- FIX START: Corrected "Select All" logic ---
    const handleSelectAllChange = (event) => {
        // Get the IDs of only the students currently visible in the filtered list
        const visibleStudentIds = new Set(filteredStudentData.map(s => s._id).filter(Boolean));

        setSelectedStudentIds(prevSelectedIds => {
            const newSelectedIds = new Set(prevSelectedIds);
            if (event.target.checked) {
                // Add all currently visible students to the master selection set
                visibleStudentIds.forEach(id => newSelectedIds.add(id));
            } else {
                // Remove all currently visible students from the master selection set
                visibleStudentIds.forEach(id => newSelectedIds.delete(id));
            }
    ...
    const renderCommonTemplate = useCallback((student, template, side) => {
    ...
    }, [idCardData, replacePlaceholders, session]);

    const renderFrontTemplate = useCallback((s) => renderCommonTemplate(s, frontTemplateToUse, 'Front'), [frontTemplateToUse, renderCommonTemplate]);
    const renderBackTemplate = useCallback((s) => renderCommonTemplate(s, backTemplateToUse, 'Back'), [backTemplateToUse, renderCommonTemplate]);

    // --- FIX START: Corrected logic for generating the final print list ---
    // This now filters the master `studentData` list, ensuring all selections are included,
    // regardless of the current view/filter on the screen.
    const studentsToPrint = useMemo(() => 
        studentData.filter(s => s?._id && selectedStudentIds.has(s._id)), 
    ...
    // --- FIX START: Logic for "Select All" checkbox state is now accurate for filtered views ---
    const numFilteredStudentsWithId = useMemo(() => filteredStudentData.filter(s => s._id).length, [filteredStudentData]);
    
    // Calculate how many of the currently visible (filtered) students are selected
    const numSelectedInFilter = useMemo(() => {
        const visibleIds = new Set(filteredStudentData.map(s => s._id).filter(Boolean));
        return [...selectedStudentIds].filter(id => visibleIds.has(id)).length;
    }, [filteredStudentData, selectedStudentIds]);

    const isSelectAllChecked = useMemo(() => numFilteredStudentsWithId > 0 && numSelectedInFilter === numFilteredStudentsWithId, [numFilteredStudentsWithId, numSelectedInFilter]);
    const isSelectAllIndeterminate = useMemo(() => numSelectedInFilter > 0 && numSelectedInFilter < numFilteredStudentsWithId, [numFilteredStudentsWithId, numSelectedInFilter]);
    // --- FIX END ---
        ...
        <>
            ...
            <Box ref={componentRef} sx={{ padding: 2, backgroundColor: '#fff', borderRadius: 1, boxShadow: 1 }}>
                ...
                <Box className="no-print" sx={{ mb: 2, minHeight: '40px', display: 'flex', alignItems: 'center', padding: '0 8px' }}>
                     ...
                     {!isLoadingData && numFilteredStudentsWithId > 0 && (<FormControlLabel control={<Checkbox checked={isSelectAllChecked} indeterminate={isSelectAllIndeterminate} onChange={handleSelectAllChange} />} label={`Select All Visible (${numFilteredStudentsWithId})`} sx={{ mr: 'auto' }} />)}
...
//     // --- FIX START: The primary bug fix is here ---
//     useEffect(() => { 
//         if (isLoadingData) return; 
//         let filtered = [...studentData];
...
//         setFilteredStudentData(filtered); 
//         // setSelectedStudentIds(new Set()); // BUG FIXED: This line was resetting selections. It has been removed.
//      }, [selectedClass, selectedSection, filterName, studentData, isLoadingData, photoFilter]);
...
//     // FIX: "Select All" now correctly adds/removes only visible students from the selection
//     const handleSelectAllChange = (event) => {
//         const visibleStudentIds = new Set(filteredStudentData.filter(s => s._id).map(s => s._id));
...
//     const renderBackTemplate = useCallback((s) => renderCommonTemplate(s, backTemplateToUse, 'Back'), [idCardData, backTemplateToUse, replacePlaceholders, session, renderCommonTemplate]);
...
//     // --- FIX: Logic for "Select All" checkbox state is now accurate for filtered views ---
//     const numFilteredStudentsWithId = useMemo(() => filteredStudentData.filter(s => s._id).length, [filteredStudentData]);

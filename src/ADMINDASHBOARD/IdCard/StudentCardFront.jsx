...
const StudentCardFront = ({ student, idCardTemplate }) => {
  ...
  const backgroundImageFront = useMemo(() => {
    return idCardTemplate?.frontImage?.url || "";
  }, [idCardTemplate]);
  const decodedApiFrontTemplate = useMemo(() => {
    if (!idCardTemplate?.frontTemplate) {
      return null;
    }
    let html = decodeBase64(idCardTemplate.frontTemplate);
    if (!html) {
      return null;
    }
  ...
  if (decodedApiFrontTemplate) {
    ...
    const hasBackgroundImage = idCardTemplate?.frontImage?.url || student?.backgroundImage || (idCardTemplate?.frontTemplate && idCardTemplate.frontTemplate.includes('${backgroundImage}'));
  ...
  return (
    <div
    //  style={{ border: '1px dashed gray', padding: '10px', width: '300px', height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', textAlign: 'center', }}
     >
...
//       .replace(/\$\{dob\}/g, student?.dateOfBirth ? moment(student.dateOfBirth).format("DD-MM-YYYY") : "N/A")
//       .replace(/\$\{mother_name\}/g, student?.motherName || "N/A")
//       .replace(/\$\{studentImage\}/g, student?.studentImage?.url || "data:image/jpeg;base64,") // Default placeholder image
//       .replace(/\$\{backgroundImage\}/g, student?.backgroundImage || backgroundImageFront || ""); // Student's specific BG > Template BG > Empty
...
// const StudentCardFront = ({ student }) => {

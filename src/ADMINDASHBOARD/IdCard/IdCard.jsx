...
// const CARD_WIDTH_MM = 54;
// const CARD_HEIGHT_MM = 86;
// // ITEMS_PER_PRINT_PAGE is now dynamic, see layoutConstants

// // Helper to chunk array for paged preview
// const chunkArray = (array, size) => {
//     if (size <= 0) return [array]; // Avoid infinite loop or errors with size 0
//     const chunked_arr = [];
//     let index = 0;
//     while (index < array.length) {
//         chunked_arr.push(array.slice(index, size + index));
...
// // Helper function to calculate layout parameters based on orientation and print mode
// const calculateLayoutConstants = (orientation, printMode) => {
//     const marginMM = 5;
//     const colGapMM = 1.75; // Default column gap
//     const rowGapMM = 5;   // Default row gap
...
//         const cardsPerRow = 5; // Based on CARD_WIDTH_MM and typical A4 landscape
//         const printAreaActualWidth = (cardsPerRow * CARD_WIDTH_MM) + ((cardsPerRow - 1) * colGapMM); // 270 + 7 = 277mm
        
//         // Calculate how many rows of single cards fit
//         const rowsPerPhysicalPage = Math.floor((pageContentHeight + rowGapMM) / (CARD_HEIGHT_MM + rowGapMM)); // e.g., 205 / 91 = 2

//         return {
//             orientation: 'landscape',
//             pageStyleSize: 'A4 landscape',
//             marginMM,
//             colGapMM,
//             rowGapMM,
//             cardsPerRow,
//             printAreaActualWidth, // The width the .id-card-print-area div should take
//             // If 'both' sides are printed for a student, assume they form one "item" vertically, taking one row of pairs.
//             itemsPerPage: printMode === 'both' ? cardsPerRow * 1 : cardsPerRow * rowsPerPhysicalPage, // 5 for 'both', 10 for 'front'/'back'
//             previewAspectRatio: `${pagePhysicalWidth} / ${pagePhysicalHeight}`,
//             justifyContentPrint: 'flex-start', // Keep existing behavior for landscape
//         };
//     } else { // portrait
//         const pagePhysicalWidth = 210;
//         const pagePhysicalHeight = 297;
//         const pageContentWidth = pagePhysicalWidth - (2 * marginMM); // 200mm
//         const pageContentHeight = pagePhysicalHeight - (2 * marginMM); // 287mm

//         // Calculate how many cards fit per row
//         const cardsPerRow = Math.floor((pageContentWidth + colGapMM) / (CARD_WIDTH_MM + colGapMM)); // e.g., 201.75 / 55.75 = 3
//         const printAreaActualWidth = (cardsPerRow * CARD_WIDTH_MM) + ((cardsPerRow - 1) * colGapMM); // 162 + 3.5 = 165.5mm

//         const rowsPerPhysicalPage = Math.floor((pageContentHeight + rowGapMM) / (CARD_HEIGHT_MM + rowGapMM)); // e.g., 292 / 91 = 3
...
//             itemsPerPage: printMode === 'both' ? cardsPerRow * 1 : cardsPerRow * rowsPerPhysicalPage, // 3 for 'both', 9 for 'front'/'back'
//             previewAspectRatio: `${pagePhysicalWidth} / ${pagePhysicalHeight}`,
//             justifyContentPrint: 'center', // Center content if print area is narrower than page content width
//         };
//     }
// };
...
//     const [printMode, setPrintMode] = useState('both');
//     const [selectedStudentIds, setSelectedStudentIds] = useState(new Set());
//     const [printOrientation, setPrintOrientation] = useState('landscape'); // New state for orientation
...
//         return calculateLayoutConstants(printOrientation, printMode);
//     }, [printOrientation, printMode]);
...
//     const handleSectionChange = (e) => setSelectedSection(e.target.value);
//     const handlePrintModeChange = (e) => setPrintMode(e.target.value);
//     const handlePrintOrientationChange = (e) => setPrintOrientation(e.target.value);
...
//     const renderFrontTemplate = useCallback((student) => {
//         const data = getStudentTemplateData(student);
//         data.backgroundImage = data.backgroundImageFront;
//         return replacePlaceholders(frontTemplateToUse, data, 'Front');
//     }, [getStudentTemplateData, frontTemplateToUse, replacePlaceholders]);

//     const renderBackTemplate = useCallback((student) => {
//         const data = getStudentTemplateData(student);
//         data.backgroundImage = data.backgroundImageBack;
//         return replacePlaceholders(backTemplateToUse, data, 'Back');
//     }, [getStudentTemplateData, backTemplateToUse, replacePlaceholders]);
...
//             if (printMode === 'back') { // This specific class for RTL printing of back cards might need review based on final layout goals
//                 printContainer.classList.add('print-rtl');
//             }
...
//                 if (index > 0 && index % layoutConstants.itemsPerPage === 0) {
//                     const pageBreakDiv = document.createElement('div');
//                     pageBreakDiv.style.pageBreakBefore = 'always';
//                     printContainer.appendChild(pageBreakDiv);
//                 }
...
//               column-gap: ${layoutConstants.colGapMM}mm !important;
//               row-gap: ${layoutConstants.rowGapMM}mm !important;
//               box-sizing: border-box !important;
//               /* If printAreaActualWidth is less than page content width, margin: auto can center it if it's a block */
//               /* For flex, justify-content:center on parent or margin: auto on flex item could work. */
//               /* Here, we set the width and rely on justify-content for horizontal alignment. */
//             }
//             .id-card-print-area.print-rtl { /* This applies if you want to reverse card order for back printing */
//               flex-direction: row-reverse !important;
//             }
//             .id-card-pair, .single-card-print {
//                 page-break-inside: avoid !important;
//                 display: block !important; /* Ensure it doesn't try to flex as an item if parent has different flex settings */
//                 width: ${CARD_WIDTH_MM}mm !important;
//                 margin: 0 !important;
//                 padding: 0 !important;
//                 border: none !important;
...
//             }
//             .id-card-pair { 
//                 /* A pair of front/back cards stacked vertically */
//                 height: auto !important; /* Height will be sum of two cards + gap */
//             }
//             .single-card-print { 
//                 height: ${CARD_HEIGHT_MM}mm !important; 
//                 overflow: hidden !important; 
...
//                 margin-bottom: 1mm !important; /* Small gap between front and back if printed together */
//             }
//              .id-card {
//                 width: ${CARD_WIDTH_MM}mm !important;
//                 height: ${CARD_HEIGHT_MM}mm !important;
//                 overflow: hidden !important;
//                 border: none !important; /* Remove any template borders for final print */
//                 box-sizing: border-box !important;
//                 display: block !important; /* Make sure it behaves as a block */
//                 background-color: transparent !important; /* Ensure template background is used */
//              }
//             .no-print, .screen-only { display: none !important; }
//           }
//         `,
...
//     const pagedFilteredStudents = useMemo(() => {
//         return chunkArray(filteredStudentData, layoutConstants.itemsPerPage > 0 ? layoutConstants.itemsPerPage : 1); // Ensure itemsPerPage is at least 1
//     }, [filteredStudentData, layoutConstants.itemsPerPage]);
...
//             <Box ref={componentRef} sx={{ padding: 2, backgroundColor: '#fff', borderRadius: 1, boxShadow: 1 }}>
//                 <div
//                     className="no-print bg-white p-2 rounded-lg shadow border border-gray-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-2 items-center mb-3" // Adjusted for more controls
//                 >
//                     <Box sx={{ flexGrow:1 }}> {/* Use grid column span or flexGrow for responsiveness */}
//                         <ReactSelect
//                             name="class"
//                             value={selectedClass}
//                             handleChange={handleClassChange}
...
//                         />
//                     </Box>
//                     <Box sx={{ flexGrow:1, minWidth: '180px' }}> {/* Ensure enough width for text field */}
//                         <TextField
//                             fullWidth
//                             id="filter-name"
//                             label="Filter by Name / Adm. No."
...
//                      <Box sx={{ flexGrow:1 }}>
//                         <FormControl fullWidth size="small" disabled={isLoadingData}>
//                             <InputLabel id="print-orientation-label">Orientation</InputLabel>
//                             <Select
//                                 labelId="print-orientation-label"
//                                 id="print-orientation-select"
//                                 value={printOrientation}
//                                 label="Orientation"
...
//                                 onChange={handleSelectAllChange}
//                             />}
//                             label={`Select All (${filteredStudentData.length} shown, ${layoutConstants.itemsPerPage} per page)`}
//                             sx={{ width: '100%' }}
//                         />
//                     )}
//                     {isLoadingData && (
...
//                     {!isLoadingData && pagedFilteredStudents.length === 0 && filteredStudentData.length > 0 && (
//                          <Typography sx={{ textAlign: 'center', width: '100%', fontStyle: 'italic' }}>
//                             Select students to preview in A4 layout. (No students selected or no matching students)
//                          </Typography>
//                     )}
//                     {pagedFilteredStudents.map((pageStudents, pageIndex) => (
//                         <div 
//                             key={`page-${pageIndex}`} 
//                             className="screen-a4-page"
//                             style={{ aspectRatio: layoutConstants.previewAspectRatio }} // Dynamic aspect ratio
//                         >
//                             <div className={`screen-id-card-layout-area ${printMode === 'back' ? 'preview-rtl' : ''}`}>
//                                 {pageStudents.map((student) => {
//                                     if (!student || !student._id) {
...
//                                         <Box 
//                                             key={studentKey} 
//                                             className="student-preview-wrapper-in-a4"
//                                             sx={{
...
//                     padding: ${layoutConstants.marginMM}mm; /* Use dynamic margin for consistency */
//                     box-sizing: border-box;
//                     max-width: 297mm; /* Max A4 dimension */
//                     width: 100%; 
//                     /* aspect-ratio is set inline now */
//                     overflow: hidden; 
//                 }
//                 .screen-id-card-layout-area {
//                     display: flex !important;
//                     flex-wrap: wrap !important;
//                     flex-direction: row !important;
//                     justify-content: ${layoutConstants.justifyContentPrint} !important; /* Match print for preview */
//                     align-content: flex-start !important;
//                     align-items: flex-start !important;
//                     width: 100% !important;
//                     height: 100% !important;
//                     column-gap: ${layoutConstants.colGapMM}mm !important; /* Dynamic gap */
//                     row-gap: ${layoutConstants.rowGapMM}mm !important;   /* Dynamic gap */
//                     box-sizing: border-box !important;
//                     overflow: hidden;
//                 }
//                 .screen-id-card-layout-area.preview-rtl {
...
// const calculateLayoutConstants = (orientation, printMode) => {
//     const marginMM = 5;
//     // CARD_WIDTH_MM and CARD_HEIGHT_MM are accessible from component scope
//     const colGapMM = 1.75; // Standard column gap
//     const rowGapMM = 5;   // Standard row gap

//     let cardsPerRow, rowsPerPhysicalPageForSingles;
//     let pagePhysicalWidth, pagePhysicalHeight;
//     let pageStyleSize;

//     if (orientation === 'landscape') {
//         pageStyleSize = 'A4 landscape';
//         pagePhysicalWidth = 297;
//         pagePhysicalHeight = 210;
//         cardsPerRow = 5; // Fixed: 5 cards per row
//         rowsPerPhysicalPageForSingles = 2; // Fixed: 2 rows for single cards
//     } else { // portrait
//         pageStyleSize = 'A4 portrait';
//         pagePhysicalWidth = 210;
//         pagePhysicalHeight = 297;
//         cardsPerRow = 2; // Fixed: 2 cards per row
//         rowsPerPhysicalPageForSingles = 5; // Fixed: 5 rows for single cards
//     }
...
//     let itemsPerPage;
//     if (printMode === 'both') {
//         // In 'both' mode, one "item" is a student (a pair of cards).
//         // We can fit one row of these pairs vertically, as a pair takes up roughly double the height of a single card.
//         itemsPerPage = cardsPerRow;
//     } else { // 'front' or 'back' mode
//         itemsPerPage = cardsPerRow * rowsPerPhysicalPageForSingles; // e.g., 5x2=10 or 2x5=10
//     }

//     // Determine justification for the print area
//     let justifyContentPrint = 'flex-start';
//     if (printAreaActualWidth < (pageContentWidth - (colGapMM * 2) )) {
//          justifyContentPrint = 'center';
//     }
//     if (orientation === 'landscape' && cardsPerRow === 5) { // Ensure 5-wide landscape is left aligned
//         justifyContentPrint = 'flex-start';
//     }
...
//               flex-direction: row !important;
//               justify-content: ${layoutConstants.justifyContentPrint} !important; 
//               align-items: flex-start !important; /* Or align-content: flex-start; */
//               width: ${layoutConstants.printAreaActualWidth}mm !important;
//               column-gap: ${layoutConstants.colGapMM}mm !important;
//               row-gap: ${layoutConstants.rowGapMM}mm !important;
//               box-sizing: border-box !important;
//               margin: 0 auto; /* Centers the block if its width is less than page content width */
//             }
//             .id-card-print-area.print-rtl {
//               flex-direction: row-reverse !important;
//             }
...
//     const pagedFilteredStudents = useMemo(() => {
//         return chunkArray(filteredStudentData, layoutConstants.itemsPerPage > 0 ? layoutConstants.itemsPerPage : 1);
//     }, [filteredStudentData, layoutConstants.itemsPerPage]);
...
//             <Box ref={componentRef} sx={{ padding: 2, backgroundColor: '#fff', borderRadius: 1, boxShadow: 1 }}>
//                 <div
//                     className="no-print bg-white p-2 rounded-lg shadow border border-gray-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 items-center mb-3"
//                 >
//                     <Box sx={{ flexGrow:1 }}>
//                         <ReactSelect
//                             name="class"
...
//                                 onChange={handleSelectAllChange}
//                             />}
//                             label={`Select All (${filteredStudentData.length} shown, ${layoutConstants.itemsPerPage > 0 ? layoutConstants.itemsPerPage : 'N/A'} per page)`}
//                             sx={{ width: '100%' }}
//                         />
//                     )}
//                     {isLoadingData && (
...
//                     {!isLoadingData && pagedFilteredStudents.length === 0 && filteredStudentData.length > 0 && (
//                          <Typography sx={{ textAlign: 'center', width: '100%', fontStyle: 'italic' }}>
//                             No students selected for print, or no students match filters for preview.
//                          </Typography>
//                     )}
//                     {pagedFilteredStudents.map((pageStudents, pageIndex) => (
//                         <div 
//                             key={`page-${pageIndex}`} 
//                             className="screen-a4-page"
//                             style={{ aspectRatio: layoutConstants.previewAspectRatio }}
//                         >
//                             <div className={`screen-id-card-layout-area ${printMode === 'back' ? 'preview-rtl' : ''}`}>
//                                 {pageStudents.map((student) => {
//                                     if (!student || !student._id) {
...
//                     box-shadow: 0 2px 8px rgba(0,0,0,0.1);
//                     margin: 0 auto 20px auto;
//                     padding: ${layoutConstants.marginMM}mm;
//                     box-sizing: border-box;
//                     max-width: 297mm; 
//                     width: 100%; 
//                     overflow: hidden; 
...
//     let cardsPerRow, rowsOfItems; // rowsOfItems can be rows of single cards or rows of pairs
//     let pageStyleSize;
...
//         pagePhysicalWidth = 297;
//         pagePhysicalHeight = 210;
//         pageContentWidth = pagePhysicalWidth - (2 * marginMM); // 287mm
//         pageContentHeight = pagePhysicalHeight - (2 * marginMM); // 200mm

//         cardsPerRow = 5; // Fixed 5 card SLOTS horizontally
        
//         if (printMode === 'both') {
//             // For pairs, assume 1 row of pairs fits. Height of a pair is approx (2*CARD_HEIGHT_MM) + 1mm internal gap.
//             // (1 * ((2*86)+1)) = 173mm. With 5mm row gap (not needed if only one row), fits in 200mm pageContentHeight.
//             rowsOfItems = 1; 
//         } else {
//             // For single cards, target 2 rows. Height is (2*CARD_HEIGHT_MM) + (1*rowGapMM) = (2*86) + 5 = 172 + 5 = 177mm. Fits in 200mm.
//             rowsOfItems = 2;
//         }
...
//         pagePhysicalWidth = 210;
//         pagePhysicalHeight = 297;
//         pageContentWidth = pagePhysicalWidth - (2 * marginMM); // 200mm
//         pageContentHeight = pagePhysicalHeight - (2 * marginMM); // 287mm

//         cardsPerRow = 2; // Fixed 2 card SLOTS horizontally

//         let itemEffectiveHeight;
//         if (printMode === 'both') {
//             itemEffectiveHeight = (2 * CARD_HEIGHT_MM) + 1; // Approx height for a pair: 173mm
//         } else {
//             itemEffectiveHeight = CARD_HEIGHT_MM; // 86mm
//         }
//         // Calculate how many rows can actually fit
//         rowsOfItems = Math.floor((pageContentHeight + rowGapMM) / (itemEffectiveHeight + rowGapMM));
//         // Ensure at least 1 row if calculation is 0 but content exists
//         if (rowsOfItems < 1) rowsOfItems = 1; 
//     }

//     const printAreaActualWidth = (cardsPerRow * CARD_WIDTH_MM) + ((cardsPerRow - 1) * colGapMM);
//     const itemsPerPage = cardsPerRow * rowsOfItems;

//     // The .id-card-print-area block will be centered using margin:auto.
//     // Inside this block, cards should always pack to the start.
//     const justifyContentPrint = 'flex-start';
...
//          <PageHeaderWithBreadcrumb breadcrumbItems={BreadcrumbList.admission} title="ID Cards"/>
//             <Box ref={componentRef} sx={{ padding: 2, backgroundColor: '#fff', borderRadius: 1, boxShadow: 1 }}>
//                 <div className="no-print bg-white p-2 rounded-lg shadow border border-gray-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 items-center mb-3">
...
//                     <Box> <FormControl fullWidth size="small" disabled={isLoadingData}> <InputLabel>Orientation</InputLabel> <Select value={printOrientation} label="Orientation" onChange={handlePrintOrientationChange}> <MenuItem value={'landscape'}>Landscape</MenuItem> <MenuItem value={'portrait'}>Portrait</MenuItem> </Select> </FormControl> </Box>
         ...
         <PageHeaderWithBreadcrumb breadcrumbItems={BreadcrumbList.admission} title="ID Cards (Landscape Only)"/>
            <Box ref={componentRef} sx={{ padding: 2, backgroundColor: '#fff', borderRadius: 1, boxShadow: 1 }}>
                ...
                <div className="no-print bg-white p-2 rounded-lg shadow border border-gray-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2 items-center mb-3">
                    ...
                    <Box> <ReactSelect name="section" value={selectedSection} handleChange={handleSectionChange} label="Section" dynamicOptions={[{ label: "All Sections", value: "" }, ...sectionOptions]} disabled={!selectedClass || sectionOptions.length === 0 || isLoadingData} placeholder="Select Section"/> </Box>
                    <Box> <TextField fullWidth label="Filter by Name / Adm. No." variant="outlined" onChange={handleFilterByNameChange} value={filterName} size="small" disabled={isLoadingData}/> </Box>
                    <Box> <FormControl fullWidth size="small" disabled={isLoadingData}> <InputLabel>Print Sides</InputLabel> <Select value={printMode} label="Print Sides" onChange={handlePrintModeChange}>
                     {/* <MenuItem value={'both'}>Both Sides</MenuItem>  */}
                     <MenuItem value={'front'}>Front Only</MenuItem> <MenuItem value={'back'}>Back Only</MenuItem> </Select> </FormControl> </Box>

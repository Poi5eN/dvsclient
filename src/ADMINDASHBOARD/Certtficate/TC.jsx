...
import { getIDcarddesign, design } from "../../Network/AdminApi";
...
const TC = () => {
    ...
    try {
      ...
      if (response?.success && response?.designFormats?.length > 0) {
        ...
        if (decodedBack) {
          ...
          console.log(
            "Using default Back Template (API decode failed or no template)"
          );
        ...
        if (fetchedData.backImage)
          setBackBackgroundImage(fetchedData.backImage?.url);
      } else {
        console.warn(
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      formDataToSend.append(
        "name",
        idCardData?.name || "Student ID Card Design"
  ...
  return (
    <>
      ...
      />
      <div>
        <div className="bg-white p-2 rounded-lg shadow border border-gray-200 w-full flex flex-row gap-2">

...
import { design, getdesignbonafideCertificate } from "../../Network/AdminApi"; // Verify path
...
const detailedSampleStudent = {
  ...
  fatherImage: {
    public_id: "",
    url: "https://via.placeholder.com/60x70/ffcccc?text=Father",
  },
  motherImage: {
    public_id: "",
    url: "https://via.placeholder.com/60x70/ccffcc?text=Mother",
  },
...
<div style='background-image:url(\${backgroundImage}); background-position: center;background-repeat: no-repeat;width: 297mm;height: 210mm;position: relative;background-size:cover;border:1px solid #ccc; box-sizing: border-box; overflow: hidden;'>
...
const SetBonafide = () => {
  ...
  const [frontBackgroundImage, setFrontBackgroundImage] = useState(
    "https://via.placeholder.com/204x325/e0f0ff?text=Front+BG"
  );
    ...
    try {
      ...
      if (response?.success && response?.designFormats?.length > 0) {
        ...
        if (decodedFront) {
          ...
          console.log(
            "Using default Front Template (API decode failed or no template)"
          );
          setFrontTemplate(defaultFrontTemplate); // Fallback to default if decode fails
        }

        if (fetchedData.frontImage)
          setFrontBackgroundImage(fetchedData?.frontImage?.url);
        //  if(fetchedData.frontImageUrl) setFrontBackgroundImage(fetchedData.frontImageUrl);
      } else {
        console.warn(
          "No custom ID card design found or API error. Using default templates."
        );
        setIdCardData(null);
  ...
  const renderTemplate = useCallback(
    (templateString, backgroundUrl, student) => {
      if (typeof templateString !== "string") {
        console.error(
          "renderTemplate called with non-string template:",
          templateString
        );
      ...
      const placeholderPersonImage =
        "https://via.placeholder.com/80x90.png?text=N/A";
      const data = {
        backgroundImage: backgroundUrl || "",
        //   backgroundImage: backgroundUrl || '',
        studentImage:
          student?.studentImage?.url ||
          "https://via.placeholder.com/85x95.png?text=No+Img",
        ...
        renderedHtml = renderedHtml.replace(
          ...
          (match, key) => {
            ...
            if (data.hasOwnProperty(cleanKey)) {
              ...
              console.warn(
                `Placeholder \${${cleanKey}} not found in data object.`
              );
  ...
  const handleFileChange = (e, side) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (side === "front") {
          setFrontBackgroundImage(reader.result);
          setUploadedFrontImageFile(file);
        }
      };
      reader.onerror = (error) => {
        ...
        if (side === "front") {
          setUploadedFrontImageFile(
            null
          ); /* Optionally reset preview: setFrontBackgroundImage(...) */
        } else {
          setUploadedBackImageFile(
            null
          ); /* Optionally reset preview: setBackBackgroundImage(...) */
        }
      };
      reader.readAsDataURL(file);
    } else {
      if (side === "front") setUploadedFrontImageFile(null);
      else setUploadedBackImageFile(null);
    }
  };
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      formDataToSend.append("name", "bonafide Certificate"); // Use fetched name or default
      formDataToSend.append("type", "bonafideCertificate");
      formDataToSend.append(
        "description",
        idCardData?.description || "Front/back design template"
      );
      formDataToSend.append(
        "isDefault",
        idCardData?.isDefault?.toString() || "true"
      ); // Send as string
      formDataToSend.append(
        "isPublic",
        idCardData?.isPublic?.toString() || "false"
      ); // Send as string
  ...
  return (
    <>
      <PageHeaderWithBreadcrumb
        ...
        title="ID Card Design Editor (Front & Back)"
      />
      <div>
        <div className="bg-white p-2 rounded-lg shadow border border-gray-200 grid sm:grid-cols-1 md:grid-cols-2 gap-2">
          <div className="bg-white  rounded-lg shadow border border-gray-200 ">
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: "300px" }}>
                <div>
                  <label htmlFor="front-bg-upload">
                    Upload Front BG Image:
                  </label>
                  <input
                    type="file"
                    id="front-bg-upload"
                ...
                <div>
                  <label htmlFor="front-template-edit">
                    Edit Front HTML Template:
                  </label>
                  <textarea
        ...
        <Button
          ...
          style={
            isSaving || isLoading ? { cursor: "not-allowed", opacity: 0.6 } : {}
          }
        />
      </div>
    </>
